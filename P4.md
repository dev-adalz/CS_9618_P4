...
## ⏰ 5-Hour Intensive Preparation Strategy {#5-hour-strategy}
...
## 💻 COMPLETE Paper 4 Coverage - All Topics {#data-structures-algorithms}

**Paper 4 Syllabus Coverage (Sections 19-20):**
- ✅ 19.1 Algorithms (Linear/Binary Search, Bubble/Insertion Sort)
- ✅ 19.1 Abstract Data Types (Stack, Queue, Linked List, Binary Tree, Dictionary)
- ✅ 19.2 Recursion (All types)
- ✅ 20.1 Programming Paradigms (OOP, Imperative - Low-level excluded)
- ✅ 20.2 File Processing (Serial, Sequential, Random access)
- ✅ 20.2 Exception Handling (Try-catch-finally)
...
```python
class Node:
    """A single box that holds data and points to the next box"""
    def __init__(self, data):
        self.data = data      # The value stored
        self.next = None      # Arrow pointing to next node


class SortedLinkedList:
    """A chain of nodes kept in sorted order (smallest to largest)"""
    ...
    def insert(self, data):
        """Add a new number in the correct sorted position"""
        new_node = Node(data)
        
        # CASE 1: List is empty OR new number is smallest
        if self.is_empty() or data < self.head.data:
            new_node.next = self.head
            self.head = new_node
            return
        
        # CASE 2: Find where to insert in the middle/end
        current = self.head
        
        # Walk until we find the right spot
    ...
    def delete(self, value):
        ...
        if self.is_empty():
            print(f"{value} not found - list is empty")
            return
    ...
    def search(self, value):
        ...
        while current:
            ...
            # Since list is sorted, if we pass the value, it's not here
            if current.data > value:
                return False
...
# Add some numbers (they'll automatically be sorted)
```python
print("Adding: 30, 10, 50, 20, 40")
my_list.insert(30)
my_list.insert(10)
my_list.insert(50)
my_list.insert(20)
my_list.insert(40)
```
# Display the list (will show sorted order)
```python
print("\nCurrent list:")
my_list.display()  # Output: [10, 20, 30, 40, 50]
```
# Check the size
```python
print(f"\nList size: {my_list.size()}")  # Output: 5
```
# Search for values
```python
print(f"\nIs 30 in the list? {my_list.search(30)}")  # True
print(f"Is 25 in the list? {my_list.search(25)}")    # False
```
# Delete a value
```
print("\nDeleting 30...")
...
```
print("\nUpdated list:")
my_list.display()  # Output: [10, 20, 40, 50]
```
# Try to delete something that doesn't exist
```
print("\nTrying to delete 100...")
my_list.delete(100)  # Output: 100 not found
```
# Add more numbers
```
print("\nAdding 15 and 35...")
...
```
print("\nFinal list:")
my_list.display()  # Output: [10, 15, 20, 40, 35, 50]
```
---
...
## 📝 Exam Day Checklist {#exam-checklist}
...
- **15-120 min:** Code Task 1 (45 min) → Task 2 (45 min) → Task 3 (15 min)
...
- ❌ Forgetting to paste code in evidence document
- ❌ Using wrong programming language
- ❌ Not testing with all required test data
- ❌ Poor variable names
- ❌ Missing comments
- ❌ Not handling exceptions
- ❌ Incorrect file operations (read/write modes)
